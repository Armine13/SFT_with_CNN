...
def input_pipeline(path, filenames, batch_size, num_epochs=None, read_threads=1):
    ...
    points_batch = tf.divide(points_batch, 35.0)
...
#            
            ...
            with tf.name_scope("costs"):
                ...
                reg_losses = tf.add_n([ tf.nn.l2_loss(v) for v in vars if (('bias' not in v.name) and ('conv' not in v.name)) ]) * 0.001
...
#                cost_test = tf.sqrt(tf.reduce_mean(tf.square(tf.sub(points, model.pred))))
                cost_train_reg = tf.sqrt(tf.reduce_mean(tf.square(tf.sub(tf.multiply(points, 35), model.pred)))) + reg_constant * reg_losses
                cost_train = tf.sqrt(tf.reduce_mean(tf.square(tf.sub(tf.multiply(points, 35), model.pred))))
                cost_test = tf.sqrt(tf.reduce_mean(tf.square(tf.sub(tf.multiply(points, 35), model.pred))))
...
#                            break
                    ...
                    coord.join(threads)
                    save_path = saver.save(sess, "checkpoints/model.ckpt")
#                    save_path = saver.save(sess, "checkpoints/"+fname+'/model.ckpt')
...
#            model = vgg16(x)
            ...
            with tf.Session(config = config) as sess:
                ...
                saver.restore(sess, "checkpoints (3rd copy)/model.ckpt")
                
#                model.load_retrained_weights(test_weights_path, sess)
                cost_test = tf.sqrt(tf.reduce_mean(tf.square(tf.sub(tf.multiply(y, 35), model.pred))))
