...
#    train_it = 100
    ...
    #Setting up FIFO queue for input data(images, points)
    ...
    with tf.Graph().as_default():
        ...
        #filenames_png = ops.convert_to_tensor(filenames_png, dtype=dtypes.string)
#        filenames_tensor = ops.convert_to_tensor(filenames, dtype=dtypes.string)
#        filename_queue = tf.train.string_input_producer(filenames_tensor,num_epochs=num_epochs, shuffle=False)
#        my_img, my_csv = readDataFromDisk(filename_queue)
#        
        images_queue, points_queue = input_pipeline(filenames, batch_size, num_epochs)
        
        #define tensor shape       
        images_queue.set_shape([224, 224, 3])
        points_queue.set_shape((3006,))
        
        image_batch, points_batch = batch_queue(images_queue, points_queue)
        #
       # train_image_batch, train_label_batch = tf.train.batch(
        #        [my_img, my_csv], batch_size=batch_size)
    ...
    #        sess = tf.Session()
            ...
            try:
                while not coord.should_stop():
                    example, coords = sess.run([image_batch, points_batch])
...
#                _, loss = sess.run([optimizer, cost], feed_dict={vgg.imgs: [train_image_batch.eval(session=sess)], y:[train_label_batch.eval(session=sess)]})
#                print("loss = {}".format(loss))
#                t = my_csv.eval()
#                print(t)
                #print(sess.run(tf.shape(my_csv)))
                #plt.figure()
                #plt.imshow(my_img.eval())
                

            
#filenames = ops.convert_to_tensor(filenames, dtype=dtypes.string)
#s = tf.read_file(filenames[0])
#f = tf.string_to_number(s, out_type=dtypes.float32)
