...
#    weights_path = 'weights/weights_latest.npz'#'weights/weights_fc_1491233872.99.npz'
    weights_path = 'weights/weights_fc_weights_latest.npz'#weights_latest__do1_2fc.npz'
    test_weights_path = 'weights/weights_1491839074.88261690.npz'
    #test_weights_path = 'weights/weights_1491839074.88523380.npz'
...
#            with tf.control_dependencies(update_ops):
			...
			#optimizer = tf.contrib.opt.ScipyOptimizerInterface(cost_train_reg, options={'maxiter':100})
				...
				try:
					...
					while not coord.should_stop():    
						...
						if step % 200 == 0:
							...
							#loss = sess.run(cost_test,feed_dict={keep_prob: 1})
							_, loss = sess.run([optimizer, cost_test],feed_dict={keep_prob: 1.0})
							...
							#optimizer.minimize(sess)
							sess.run(optimizer,feed_dict={keep_prob: 1.0})#, feed_dict={x: images_batch_train.eval(), y: points_batch_train.eval()})
						#Test if epoch ended
