...
#        self.parameters = []
        ...
        with tf.name_scope('conv1_1') as scope:
            self.conv1_1_kernel = tf.Variable(tf.truncated_normal([3, 3, 3,256], dtype=tf.float32,
                                                     stddev=1e-1), trainConv, name='weights')
            conv = tf.nn.conv2d(self.imgs, self.conv1_1_kernel, [1, 1, 1, 1], padding='SAME')
            self.conv1_1_biases = tf.Variable(tf.constant(0.0, shape=[256], dtype=tf.float32),
        ...
        with tf.name_scope('conv1_2') as scope:
            self.conv1_2_kernel = tf.Variable(tf.truncated_normal([3, 3,256 , 256], dtype=tf.float32,
                                                     stddev=1e-1), trainConv, name='weights')
            conv = tf.nn.conv2d(self.conv1_1, self.conv1_2_kernel, [1, 1, 1, 1], padding='SAME')
            self.conv1_2_biases = tf.Variable(tf.constant(0.0, shape=[256], dtype=tf.float32),
                                 trainConv, name='biases')
        ...
        with tf.name_scope('conv2_1') as scope:
            self.conv2_1_kernel = tf.Variable(tf.truncated_normal([3, 3, 256, 256], dtype=tf.float32,
                                                     stddev=1e-1), trainConv, name='weights')
            conv = tf.nn.conv2d(self.pool1, self.conv2_1_kernel, [1, 1, 1, 1], padding='SAME')
            self.conv2_1_biases = tf.Variable(tf.constant(0.0, shape=[256], dtype=tf.float32),
                                 trainConv, name='biases')
        ...
        with tf.name_scope('conv2_2') as scope:
            self.conv2_2_kernel = tf.Variable(tf.truncated_normal([3, 3, 256, 256], dtype=tf.float32,
                                                     stddev=1e-1), trainConv, name='weights')
            conv = tf.nn.conv2d(self.conv2_1, self.conv2_2_kernel, [1, 1, 1, 1], padding='SAME')
            self.conv2_2_biases = tf.Variable(tf.constant(0.0, shape=[256], dtype=tf.float32),
                                 trainConv, name='biases')
        ...
        with tf.name_scope('conv3_1') as scope:
            self.conv3_1_kernel = tf.Variable(tf.truncated_normal([3, 3, 256, 256], dtype=tf.float32,
                                                     stddev=1e-1), trainConv, name='weights')
...
#        print("Weights loaded")
    ...
    def load_retrained_weights(self, weight_file, sess):
        ...
        a = 0
        for k in keys:
            if k == 'fc8_b' or k == 'fc8_W':
                continue
#            if a >= 18: ################################
            if k =='fc1_W' or k=='fc1_b' or k =='fc2_W' or k=='fc2_b' or k =='fc3_W' or k=='fc3_b':
                sess.run(self.retrained_parameters[a].assign(weights[k]))
            a += 1
        print("Weights loaded")
