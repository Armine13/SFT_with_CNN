...
######## Focal length prediction
        with tf.name_scope('fc_fl_1') as scope:
            shape = int(np.prod(self.pool5.get_shape()[1:]))
            self.fcfl_1w = tf.Variable(tf.random_normal([shape, 1024],
                                                dtype=tf.float32)*tf.sqrt(2.0/1024),
                                                trainable=self.trainFl, name='weights')
            self.fcfl_1b = tf.Variable(tf.constant(0, shape=[1024], dtype=tf.float32),
        ...
        with tf.name_scope('fc_fl_2') as scope:
            self.fcfl_2w = tf.Variable(tf.random_normal([1024, 1024],
                                                dtype=tf.float32),
                                                trainable=self.trainFl, name='weights')
            self.fcfl_2b = tf.Variable(tf.constant(0, shape=[1024], dtype=tf.float32),
                               trainable=self.trainFl, name='biases')
        ...
        with tf.name_scope('fc_fl_3') as scope:
            self.fcfl_3w = tf.Variable(tf.random_normal([1024, 1],
                                                dtype=tf.float32),
                                                trainable=self.trainFl, name='weights')
            self.fcfl_3b = tf.Variable(tf.constant(0, shape=[1], dtype=tf.float32),
                               trainable=self.trainFl, name='biases')
            
            self.pred_fl = tf.nn.bias_add(tf.matmul(fcfll1, self.fcfl_3w), self.fcfl_3b)
            
            self.retrained_parameters +=[self.fcfl_3w, self.fcfl_3b]


########## detector
        with tf.name_scope('fc_1_detector') as scope:
            shape = int(np.prod(self.pool5.get_shape()[1:]))
            self.fcdet_1w = tf.Variable(tf.random_normal([shape, 1024],
                                                dtype=tf.float32)*tf.sqrt(2.0/1024),
                                                trainable=self.trainDet, name='weights')
            self.fcdet_1b = tf.Variable(tf.constant(0, shape=[1024], dtype=tf.float32),
                               trainable=self.trainDet, name='biases')
            pool5_flat = tf.reshape(self.pool5, [-1, shape])
            
            self.fc1_det = tf.nn.relu(tf.nn.bias_add(tf.matmul(pool5_flat, self.fcdet_1w), self.fcdet_1b))
            
            self.retrained_parameters +=[self.fcdet_1w, self.fcdet_1b]
            
        with tf.name_scope('fc_2_detector') as scope:
            
            self.fcdet_2w = tf.Variable(tf.random_normal([1024, 2],
                                                dtype=tf.float32)*tf.sqrt(2.0/2),
                                                trainable=self.trainDet, name='weights')
            self.fcdet_2b = tf.Variable(tf.constant(0, shape=[2], dtype=tf.float32),
                               trainable=self.trainDet, name='biases')
            
            self.detected = tf.nn.bias_add(tf.matmul(self.fc1_det, self.fcdet_2w), self.fcdet_2b)
            
#            self.detected = tf.transpose(self.detected)
            
            self.retrained_parameters +=[self.fcdet_2w, self.fcdet_2b]
