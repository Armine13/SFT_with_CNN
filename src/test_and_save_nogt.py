...
def getFileList(datapath):
    file_names_png = glob1(datapath ,"*.png")
#    file_list_csv = [os.path.join(datapath, fname) for fname in file_names_csv]
    file_list_png = [os.path.join(datapath, fname) for fname in file_names_png] #remove extension
    return file_list_png

test_weights_path = 'weights/weights_def_all_best_2.7_0.16.npz'
#testpath = "/home/isit/armine/Dropbox/datasets/fl/rig_rt+fl/test/"
testpath = "/home/isit/armine/Dropbox/american_pillow_gt_def_square/"

filenames_test = getFileList(testpath)
...
#            arr = str(text).split(',')
#            pts = arr[2:]
            imname = filenames_test[i]
            im_cont = tf.read_file(imname)
...
#                data['gt_fl'][step] = fl_test
                data['pred'][step,:] = model.pred.eval(feed_dict={x: image_test})
                data['pred_fl'][step] = model.pred_fl.eval(feed_dict={x: image_test})
