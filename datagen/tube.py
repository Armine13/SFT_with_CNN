...
outdir = '/home/arvardaz/Dropbox/datasets/tube/train/'
outdir_test = '/home/arvardaz/Dropbox/datasets/tube/test/'
...
bpy.data.scenes['Scene'].cycles.samples = 1200
...
#    obj.parent.pose.bones['Bone.005'].location = loc + Vector((np.random.uniform(-3,3),np.random.uniform(-3,3),np.random.uniform(-5,-3)))
    bpy.data.objects['Armature'].pose.bones['Bone.005'].location = Vector((np.random.uniform(-4,4),np.random.uniform(0,-4),np.random.uniform(-4,4)))
...
#    randomRotateTranslate(obj, 8)#std=3
    randomRotateTranslate(obj, 1, 3)#1.5, 2)#4, 10)#std=3
