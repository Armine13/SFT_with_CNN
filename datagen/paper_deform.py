...
def randomRotateTranslate(meshObj, s, d):
    ...
    #Rotation
    alpha = np.random.uniform(-70, 70)
    beta = np.random.uniform(-70, 70)
    gamma = np.random.uniform(-70, 70)
    ...
    for window in bpy.context.window_manager.windows:
        for area in window.screen.areas:
            if area.type == 'VIEW_3D':
                override = bpy.context.copy()
                override['area'] = area
                bpy.ops.object.modifier_apply(override, apply_as='DATA',modifier='Wave')
                

def paperDeform(obj, d, x, y, off, n):
    obj.modifiers["Wave"].damping_time = 0#10
    obj.modifiers["Wave"].falloff_radius = 0
    obj.modifiers["Wave"].speed = 0.25
    obj.modifiers["Wave"].width = 2.8
    
    obj.modifiers["Wave"].height = 0.5 * d # (-1)**np.random.randint(1,3)*0.5
    obj.modifiers["Wave"].start_position_x = x # (-1)**np.random.randint(1,3)*np.random.uniform(3, 23)
    obj.modifiers["Wave"].start_position_y = y #(-1)**np.random.randint(1,3)*np.random.uniform(2, 23)
    obj.modifiers["Wave"].narrowness = n # np.random.uniform(0, 1.51)
    obj.modifiers["Wave"].time_offset = off# np.random.uniform(-200,-60)
...
outdir = '/home/arvardaz/Dropbox/datasets/paper_deform/'
...
bpy.ops.wm.open_mainfile(filepath = '/home/arvardaz/SFT_with_CNN/datagen/paper_def.blend')
...
for i in np.arange(iters):
    ...
    a = np.random.uniform(-70, 70); b = np.random.uniform(-70, 70); g = np.random.uniform(-70, 70)
    x = np.random.uniform(-1.5, 1.5); y = np.random.uniform(-5, 5); z = np.random.uniform(-1.5, 1.5)
    
#    def_a, def_c = np.random.uniform(-3.5, 3.5), np.random.uniform(-3.5, 3.5)
#    def_range = np.linspace(-0.2, -4, frames)
    
    d = (-1)**np.random.randint(1,3)
    x_mod = (-1)**np.random.randint(1,3)*np.random.uniform(3, 23)
    y_mod = (-1)**np.random.randint(1,3)*np.random.uniform(2, 23)
    off = np.random.uniform(-200,-60)
    
    n_range = np.linspace(0, 1.18, frames)#np.linspace(0, 1.51, frames)
...
#        paperDeform(obj, a)
        paperDeform(obj, d, x_mod, y_mod, off, n)
        applyWaveModifier(obj)
...
#        randomRotateTranslate(obj, 1.5, 5)#1.5, 2)#4, 10)#std=3
...
#    if np.random.randint(10) is not 0:
#            
#        if i % 10 != 0:
...
#            bpy.context.scene.render.filepath = outdir + '{}_{:04}.png'.format(filename, i)
#            bpy.ops.render.render(write_still=True)
#            saveEmpty(cam, '{}_{:04}.csv'.format(filename, i), outdir, False)
#        else:
#            bpy.context.scene.render.filepath = outdir_test + '{}_{:04}.png'.format(filename, i)
#            bpy.ops.render.render(write_still=True)
#            saveEmpty(cam, '{}_{:04}.csv'.format(filename, i), outdir_test, False)
#    bpy.ops.ptcache.free_bake_all()
#    bpy.data.objects['backup.001'].parent.pose.bones['Bone.005'].location += Vector((-4,0,-2))
