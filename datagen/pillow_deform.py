...
bpy.ops.wm.open_mainfile(filepath = '/home/arvardaz/SFT_with_CNN/datagen/pillow_def.blend')
...
loc = bpy.data.objects['backup'].parent.pose.bones['Bone.001'].location
...
for i in np.arange(iters):
    ...
    lim = [np.random.uniform(-55, 55), np.random.uniform(-55, 55), np.random.uniform(-55, 55)]
    
    k = np.random.randint(0, 2)
    lim[k] = (-1)**np.random.randint(1,3)*55
    alpha_range = np.zeros((frames, 3))
#    alpha_range[:,0] = lim[0]
#    alpha_range[:,1] = lim[1]
#    alpha_range[:,2] = lim[2]
#    for k in range(3):
#    alpha_range[:,k] = np.linspace(0,lim[k],frames)
    alpha_range[:,0] = np.linspace(0,lim[0],frames)
    alpha_range[:,1] = np.linspace(0,lim[1],frames)
...
#        bpy.data.objects['Armature'].pose.bones['Bone.005'].location = Vector((def_a, d, def_c))
        ...
        obj.parent.pose.bones['Bone.001'].rotation_euler = Euler((np.deg2rad(alpha_range[j,0]), np.deg2rad(alpha_range[j,1]), np.deg2rad(alpha_range[j,2])))
#        break
