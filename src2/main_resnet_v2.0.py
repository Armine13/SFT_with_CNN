from resnet_model_v2 import resnet
...
#            test_loss = cost.eval(feed_dict={x: image_test, y:points_test,keep_prob: 1.0})
            ...
            pr = model.pred_fl.eval(feed_dict={x: image_test, fl: fl_test, presence: presence_test, y:points_test})
...
#    trainpath = "/home/isit/armine/Dropbox/tube_test/train"
#    testpath = "/home/isit/armine/Dropbox/tube_test/test"

#    weights_path = 'weights/weights_latest (another copy).npz'
#    test_weights_path = 'weights/weights_latest.npz'#weights_bg_best_1.56.npz'#weights_1492787682.16_best.npz'#weights_latest_main.npz'
    check_path = 'checkpoints/model_3d.ckpt'
    
    #params
    learning_rate = 0.1 #0.000008
    #do_train = 1.0
#    lr_decay = 0.9
    reg_constant = 0#0.23
...
#                filenames_train = filenames_train[:35]
                ...
                images = tf.placeholder_with_default(images_batch_train, shape=[batch_size, 224, 224, 3])
                points = tf.placeholder_with_default(points_batch_train, shape=[batch_size, 3006])
                fl = tf.placeholder_with_default(fl_batch_train, shape=[batch_size])
                presence = tf.placeholder_with_default(presence_batch_train, shape=[batch_size])
                
#                images = tf.placeholder(tf.float32, shape=[None, 224, 224, 3])
#                points = tf.placeholder(tf.float32, shape=[None, 3006])
#                fl = tf.placeholder(tf.float32, shape=None)
                
            model = resnet(images, batch_size, 'train')
...
#                print(model.pred_fl.get_shape())
#                print(fl.get_shape())

#                print(model.detected[:,0].get_shape())
#                l = (tf.nn.softmax_cross_entropy_with_logits(labels=presence_one_hot,logits=model.detected)) +rmse(points, model.pred) #+ tf.divide(tf.abs(fl-tf.reshape(model.pred_fl, [batch_size])),fl)
#                print(tf.abs(tf.nn.sigmoid(model.detected)-1).get_shape())
#                print(rmse(points, model.pred).get_shape())
#                print(model.detected.get_shape())
#                l = tf.abs(tf.reshape(model.detected,[batch_size])-1) + rmse(points, model.pred)# + tf.divide(tf.abs(fl-tf.reshape(model.pred_fl, [batch_size])),fl)
#                d = tf.reshape(model.detected,[batch_size])#l2(tf.nn.sigmoid(model.detected))
#                d = (tf.nn.softmax_cross_entropy_with_logits(labels=presence_one_hot,logits=model.detected))
#                d = 
#                print(d.get_shape())
#                cost_train_reg = tf.reduce_mean(tf.where(tf.cast(presence,tf.bool), d, l))
                cost_train_reg = tf.reduce_mean(rmse(points, model.pred))+tf.reduce_mean(tf.divide(tf.abs(fl - tf.reshape(model.pred_fl, [batch_size])), fl))
...
#                cost_train_reg = tf.contrib.losses.absolute_difference(model.pred, points)
                cost_test = tf.reduce_mean(rmse(points, model.pred))
...
#                cost_test = rmse(tf.multiply(points, 227), tf.multiply(model.pred, 227))
                cost_train_fl = tf.reduce_mean(tf.divide(tf.abs(fl - tf.reshape(model.pred_fl, [batch_size])), fl))#tf.reduce_mean(tf.divide(tf.abs(fl - model.pred_fl), fl))
...
#                optimistic_restore(sess, "checkpoints/")
#                optimistic_restore(sess, check_path) #---------------------------------------------------------------------------------------
...
#                            _, loss, summary = sess.run([optimizer, cost_train, summary_op], options=tf.RunOptions(trace_level=tf.RunOptions.FULL_TRACE),run_metadata=run_metadata)
                            _, loss, acc_train, summary  = sess.run([optimizer, cost_train, accuracy, summary_op])
...
#                            im_test, fl_test, presence_test, pts_test = sess.run([images_batch_train, fl_batch_train, presence_batch_train, points_batch_train])
#                            summary, loss_test = sess.run([val_summary, cost_test], feed_dict={images: im_test, points: pts_test}, options=tf.RunOptions(trace_level=tf.RunOptions.FULL_TRACE),run_metadata=run_metadata)
                            summary, loss_test, loss_test_fl, acc_val = sess.run([val_summary, cost_test, cost_test_fl, accuracy], feed_dict={images: im_test, fl:fl_test, presence:presence_test, points: pts_test})
                            epoch_acc.append(acc_val)
                            
                            val_writer.add_summary(summary, step)
                            epoch_test_losses.append(loss_test)
                            epoch_test_losses_fl.append(loss_test_fl)
                            
							
                            #print('Step %d: train_loss = %.2f (%f sec)' % (step, loss, time.time() - start_time))
#                            print('Step %d: train_loss = %.2f val_loss = %.2f fl_e = %.2f test_acc = %.2f (%f s) d = %.2f' % (step, loss, loss_test, loss_test_fl, acc_val, time.time() - start_time, presence_test))
                            print('Step %d: train_loss = %.2f val_loss = %.2f fl_e = %.2f test_acc = %.2f (%f s)' % (step, loss, loss_test, loss_test_fl, acc_val, time.time() - start_time))
#                            print(pres_val)
...
#                            sess.run(optimizer, options=tf.RunOptions(trace_level=tf.RunOptions.FULL_TRACE),run_metadata=run_metadata)#, feed_dict={x: images_batch_train.eval(), y: points_batch_train.eval()})
                        ...
                        if step > 0 and (step % int(train_size / batch_size) == 0):
                            with tf.device('/cpu:0'):
                                print("Epoch-- train: {} test: {} fl: {} test_accuracy: {} Time elapsed: {} min".format(np.mean(epoch_losses),np.mean(epoch_test_losses),np.mean(epoch_test_losses_fl),np.mean(epoch_acc),(time.time() - epoch_start_time)/60.0))
...
#                                    saveWeights(model, 'latest_main', True)
                                    save_path = saver.save(sess, check_path)
...
#                    saveWeights(model, 'latest_main', True)
    ...
    if test:
        ...
        with tf.Graph().as_default():
            ...
            y = tf.placeholder(tf.float32, [batch_size, 3006])
            fl = tf.placeholder(tf.float32, shape=batch_size)
            presence = tf.placeholder(tf.float32, shape=batch_size)
#            keep_prob = tf.placeholder(tf.float32)
#            vgg = vgg16(x, keep_prob)
            model = resnet(x, batch_size, 'eval')
            config = tf.ConfigProto()
            config.gpu_options.allocator_type = 'BFC'
