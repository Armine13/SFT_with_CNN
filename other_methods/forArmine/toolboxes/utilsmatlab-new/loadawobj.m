function [V,F3,F4,VT,F3T,mtlFile,usemtl,mtlNames]=loadawobj(modelname)
%modified by collins
...
while Lyn>=0
  ...
  if l==0  % isempty(s) ; 
    ...
    case 'usemtl' % what is this??
      ...
      [mtlID,mtlNames] = getMtlID(mtlNames,usemtl);
    case 'v' % vertex
      v=sscanf(Lyn(2:l),'%f');
  
%       V(:,vnum)=v(1:3); % initialy
      V(:,vnum)=v(1:3); % modified by Mathias
...
function [mtlID,mtlNames] = getMtlID(mtlNames,mtlName)
if isempty(mtlNames)
    mtlID=1;
    mtlNames{1} = mtlName;
    
else
    for i=1:length(mtlNames)
       if strcmp(mtlName,mtlNames{i})
           mtlID = i;
           return;
       end
    end
    mtlNames = [mtlNames,mtlName];
    mtlID = length(mtlNames);
end