...
static
void interpolate_nearest(double *pO, const double *pF, const double *pX, const double *pY, const mwSize ND, const mwSize M, const mwSize N, const mwSize P) {
  const mwSize LO = M*N;
  for (mwSize i=0; i<ND; ++i) {
    const double &x = pX[i];
    const double &y = pY[i];

    const int x_round = int(round2(x))-1;
    const int y_round = int(round2(y))-1;
...
template <mwSize P>
static
void interpolate_nearest_unrolled(double *pO, const double *pF, const double *pX, const double *pY, const mwSize ND, const mwSize M, const mwSize N) {
  const mwSize LO = M*N;
  for (mwSize i=0; i<ND; ++i) {
    const double &x = pX[i];
    const double &y = pY[i];
...
static
void interpolate_linear(double *pO, const double *pF, const double *pX, const double *pY, const mwSize ND, const mwSize M, const mwSize N, const mwSize P) {
  const mwSize LO = M*N;
  for (mwSize i=0; i<ND; ++i) {
    const double &x = pX[i];
    const double &y = pY[i];
...
template <mwSize P>
static
void interpolate_linear_unrolled(double *pO, const double *pF, const double *pX, const double *pY, const mwSize ND, const mwSize M, const mwSize N) {
  const mwSize LO = M*N;
  for (mwSize i=0; i<ND; ++i) {
    const double &x = pX[i];
    const double &y = pY[i];
...
static
void interpolate_bicubic(double *pO, const double *pF, const double *pX, const double *pY, const mwSize ND, const mwSize M, const mwSize N, const mwSize P) {
  const mwSize LO = M*N;
  for (mwSize i=0; i<ND; ++i) {
    const double &x = pX[i];
    const double &y = pY[i];
...
template <size_t P>
static
void interpolate_bicubic_unrolled(double *pO, const double *pF, const double *pX, const double *pY, const mwSize ND, mwSize M, mwSize N) {
  const mwSize LO = M*N;
  for (mwSize i=0; i<ND; ++i) {
    const double &x = pX[i];
    const double &y = pY[i];
...
void mexFunction(int nlhs, mxArray *plhs[],
  ...
  if (!(mxIsDouble(prhs[0]) && mxIsDouble(prhs[1]) && mxIsDouble(prhs[2])))
    mexErrMsgTxt("ba_interp2 takes only double arguments for IMG,X,Y");
  ...
  plhs[0] = mxCreateNumericArray(mxGetNumberOfDimensions(prhs[2]) + mxGetNumberOfDimensions(prhs[0]) - 2, outDims, mxDOUBLE_CLASS, mxREAL);

  const mwSize ND = mxGetNumberOfElements(prhs[1]);

  const double *pF = mxGetPr(prhs[0]);
  const double *pX = mxGetPr(prhs[1]);
  const double *pY = mxGetPr(prhs[2]);
  double       *pO = mxGetPr(plhs[0]);  

  switch(parseInterpolationMethod(nrhs, prhs)) {
    case Nearest:
      switch (P) {
        case 1: interpolate_nearest_unrolled<1>(pO, pF, pX, pY, ND, M, N); break;
        case 2: interpolate_nearest_unrolled<2>(pO, pF, pX, pY, ND, M, N); break;
        case 3: interpolate_nearest_unrolled<3>(pO, pF, pX, pY, ND, M, N); break;
        case 4: interpolate_nearest_unrolled<4>(pO, pF, pX, pY, ND, M, N); break;
        case 5: interpolate_nearest_unrolled<5>(pO, pF, pX, pY, ND, M, N); break;
        case 6: interpolate_nearest_unrolled<6>(pO, pF, pX, pY, ND, M, N); break;
        case 7: interpolate_nearest_unrolled<7>(pO, pF, pX, pY, ND, M, N); break;
        case 8: interpolate_nearest_unrolled<8>(pO, pF, pX, pY, ND, M, N); break;
        case 9: interpolate_nearest_unrolled<9>(pO, pF, pX, pY, ND, M, N); break;
        default: 
                interpolate_nearest(pO, pF, pX, pY, ND, M, N, P);
      }
      break;
    case Linear:
      switch (P) {
        case 1: interpolate_linear_unrolled<1>(pO, pF, pX, pY, ND, M, N); break;
        case 2: interpolate_linear_unrolled<2>(pO, pF, pX, pY, ND, M, N); break;
        case 3: interpolate_linear_unrolled<3>(pO, pF, pX, pY, ND, M, N); break;
        case 4: interpolate_linear_unrolled<4>(pO, pF, pX, pY, ND, M, N); break;
        case 5: interpolate_linear_unrolled<5>(pO, pF, pX, pY, ND, M, N); break;
        case 6: interpolate_linear_unrolled<6>(pO, pF, pX, pY, ND, M, N); break;
        case 7: interpolate_linear_unrolled<7>(pO, pF, pX, pY, ND, M, N); break;
        case 8: interpolate_linear_unrolled<8>(pO, pF, pX, pY, ND, M, N); break;
        case 9: interpolate_linear_unrolled<9>(pO, pF, pX, pY, ND, M, N); break;
        default: 
                interpolate_linear(pO, pF, pX, pY, ND, M, N, P);
      }
      break;
    case Cubic:
      switch (P) {
        case 1: interpolate_bicubic_unrolled<1>(pO, pF, pX, pY, ND, M, N); break;
        case 2: interpolate_bicubic_unrolled<2>(pO, pF, pX, pY, ND, M, N); break;
        case 3: interpolate_bicubic_unrolled<3>(pO, pF, pX, pY, ND, M, N); break;
        case 4: interpolate_bicubic_unrolled<4>(pO, pF, pX, pY, ND, M, N); break;
        case 5: interpolate_bicubic_unrolled<5>(pO, pF, pX, pY, ND, M, N); break;
        case 6: interpolate_bicubic_unrolled<6>(pO, pF, pX, pY, ND, M, N); break;
        case 7: interpolate_bicubic_unrolled<7>(pO, pF, pX, pY, ND, M, N); break;
        case 8: interpolate_bicubic_unrolled<8>(pO, pF, pX, pY, ND, M, N); break;
        case 9: interpolate_bicubic_unrolled<9>(pO, pF, pX, pY, ND, M, N); break;
        default: 
                interpolate_bicubic(pO, pF, pX, pY, ND, M, N, P);
      }
      break;
    default:
