...
%% load cvlab paper dataset

%frame_number = 1;

for frame_number = 1:190

frame_number,    
    
load(['./cvlab-dataset/input',num2str(frame_number),'.mat']);  
GTHm = input.object_shape3D; % ground truth (m)
GTH = 1000*GTHm;

%% data processing with warp
K = input.K;
pixels = input.pixels;
n_particles = size(pixels,2);    % number of particles
fx = K(1,1);  fy = K(2,2);
uo = K(1,3);  vo = K(2,3);
m = pixels - repmat( [uo;vo], [1, n_particles] );
xy = m ./ repmat( [fx;fy], [1, n_particles] ); 
p = 1000*input.template(1:2,:);  % template
q = xy;  % image points  camera frame
...
save(['./basin-data/basin_cvlab_frame',num2str(frame_number)],'RMSE_BRUNET','RMSE_EROL','TICTOC_BRUNET','TICTOC_EROL');
