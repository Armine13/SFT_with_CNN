...
%             Qwpn = Shapefrom2Normals(N1o,N2o,p,proi,qw,Q,options.phi.nC,options.phi.er);
    ...
    case {'BGCC12C','BGCC12CR'}
        if(~(isfield(options.phi,'ctrlpts') && strcmp(upper(options.method),'BGCC12CR'))) % Is user providing with an initialization of phi ?
            ...
            I = I(:)';
            [bbs1, ctrlpts1] = make_bbs_warp(proi,I,options.phi.nC,options.phi.er*10,options.KLims);
            
            IU = bbs_eval(bbs1, ctrlpts1, proi(1,:)',proi(2,:)',1,0);
            IV = bbs_eval(bbs1, ctrlpts1, proi(1,:)',proi(2,:)',0,1);
            IU = IU(:);
            IV = IV(:);                        
            ...
            for kc2=1:size(ncombinations2,1)
                ...
                for kc=1:size(ncombinations,1)
                    ...
                    if(length(dgth)==0)
                        ...
                        ctrlptsint = BBSIntegration(bbs1,proi,Jmu2,options.phi.er/10000);
                        muw = bbs_eval(bbs1,ctrlptsint,proi(1,:)',proi(2,:)',0,0); 
                        ...
                        [bbs3,ctrlpts3] = make_bbs_warp(proi,Q,options.phi.nC,options.phi.er,options.KLims);                        
                        %[~,Qw]=TPSWarpDiff(proi,L2,C,options.phi.ir,EpsilonLambda);
                        out.phi{kc3}.Q=Q;
                        out.phi{kc3}.p=proi;
                        out.phi{kc3}.bbs=bbs3;
                        out.phi{kc3}.ctrlpts=ctrlpts3;                                                
                        out.phi{kc3}.er=options.phi.er;
                        out.phi{kc3}.I=I;
                        options.phic{kc3}.bbs = bbs3;
                        options.phic{kc3}.ctrlpts = ctrlpts3;
                        switch(options.method)
                            case 'BGCC12CR'                                                                                                
                                out = NLrefine_ris(options.phic{kc3}.bbs, options.phic{kc3}.ctrlpts, p, q, ...
                                options.rephi.iso, options.rephi.bend);
                        end
                        kc3=kc3+1;
                    else
            ...
            switch(options.method)
                case 'BGCC12CR'
                    out = NLrefine_ris(options.phic.bbs, options.phic.ctrlpts, p, q, ...
                                options.rephi.iso, options.rephi.bend);
            end
        end
