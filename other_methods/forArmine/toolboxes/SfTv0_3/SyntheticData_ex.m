...
load ../Data/savedData/scene1s-factor1exp1N100sigma1.mat;        
p = data.p;
q = data.q;
P2 = data.P2;
...
% options.method = 'CPBC15IR';
% % outd = SfTi(proi,qw,Jw,options);
% outn = SfT(p,q,options);
% Qn = bbs_eval(outn.phi.bbs,outn.phi.ctrlpts,p(1,:)',p(2,:)',0,0); 

% Conformal reconstruction
% options.phigth = outn.phi;
options.method = 'BGCC12C';
outc = SfT(p,q,options);
Qc = bbs_eval(outc.phi{2}.bbs,outc.phi{2}.ctrlpts,p(1,:)',p(2,:)',0,0); 
Qc = RegisterToGTH(Qc,P2);

figure,
plot3(Qc(1,:),Qc(2,:),Qc(3,:),'bo');
hold on;
