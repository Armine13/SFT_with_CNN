...
#    ax.plot(im[:,0], im[:,1], 'bx')
    ...
    ax.plot(im[:,0], im[:,1],ls = 'none', c='#50BFE6', marker='o',markersize=4,linewidth=0.0,alpha=0.3)#msize=2
    ax.set_xlim([0,224])
    ax.set_ylim([0,224])
    ax.axis("off")
...
#        plt.show()
        plt.savefig(outdir+"points_{}.png".format(k), bbox_inches='tight')
