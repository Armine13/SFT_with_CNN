from vgg16_model_do1_3fc import vgg16
...
#    weights_path = 'weights/weights_latest.npz'#'weights/weights_fc_1491233872.99.npz'
    weights_path = 'weights/weights_1491630932.19_latest__do1_3fc.npz'
    test_weights_path = 'weights/weights_1491630932.19_latest__do1_3fc.npz'
...
#            with tf.control_dependencies(update_ops):
            ...
            with tf.Session(config = config) as sess:
                ...
                #        im_train, pts_train = sess.run([images_batch_train, points_batch_train])
                        ...
                        if step % 200 == 0:
                            _, loss = sess.run([optimizer, cost_test],feed_dict={keep_prob: 1})#, feed_dict={x: images_batch_train.eval(), y: points_batch_train.eval()})
                            ...
                            sess.run(optimizer,feed_dict={keep_prob: 1})#, feed_dict={x: images_batch_train.eval(), y: points_batch_train.eval()})
                        #Test if epoch ended
                    ...
                    saveWeights(vgg, 'latest__do1_3fc', True)
                    np.savetxt('weights/do1_3fc_losses.txt', all_epoch_losses)
