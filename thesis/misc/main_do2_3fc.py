from vgg16_model_do2_3fc import vgg16
...
#    weights_path = 'weights/weights_latest.npz'#'weights/weights_fc_1491233872.99.npz'
    weights_path = 'weights/weights_1491574712.33.npz'
    test_weights_path = 'weights/weights_latest.npz'
...
#            with tf.control_dependencies(update_ops):
            ...
            with tf.Session(config = config) as sess:
                ...
                #        im_train, pts_train = sess.run([images_batch_train, points_batch_train])
                        ...
                        if step % 200 == 0:
                            _, loss = sess.run([optimizer, cost_test],feed_dict={keep_prob: 0.85})#, feed_dict={x: images_batch_train.eval(), y: points_batch_train.eval()})
                            ...
                            sess.run(optimizer,feed_dict={keep_prob: 0.85})#, feed_dict={x: images_batch_train.eval(), y: points_batch_train.eval()})
                        #Test if epoch ended
