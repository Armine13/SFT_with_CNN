...
#                               name='pool4')
    ...
    def fc_layers(self):
        ...
        with tf.name_scope('fc1') as scope:
            ...
            self.fc1w = tf.Variable(tf.truncated_normal([shape, 1024],
                                                         dtype=tf.float32,
                                                         stddev=1e-1), trainable=self.trainFc, name='weights')
            self.fc1b = tf.Variable(tf.constant(1.0, shape=[1024], dtype=tf.float32),
                                 trainable=self.trainFc, name='biases')
            pool4_flat = tf.reshape(self.pool4, [-1, shape])
            fc1l = tf.nn.bias_add(tf.matmul(pool4_flat, self.fc1w), self.fc1b)
...
#            self.retrained_parameters += [self.fc2w, self.fc2b]
        ...
        with tf.name_scope('fc3') as scope:
            ...
            self.fc3w = tf.Variable(tf.random_normal([1024, 3006],
...
#            tf.summary.histogram("out_fc3b", self.fc3b)
    ...
    def load_retrained_weights(self, weight_file, sess):
        ...
        skiplist = ['conv5_1_Kernel', 'conv5_1_biases', 'conv5_2_Kernel', 'conv5_2_biases', 'conv5_3_Kernel', 'conv5_3_biases', 'fc2_b', 'fc2_W']
        skiplist2 = ['fc1_b', 'fc1_W','fc3_b', 'fc3_W']########################################################] ################################################################
        a = 0
        for k in keys:
            if k in skiplist:
                continue
            if k not in skiplist2:
                sess.run(self.retrained_parameters[a].assign(weights[k]))
            a += 1
        print("Weights loaded")
        
    def save_weights(self, fname):
        conv_l = [(1, 1), (1, 2), (2, 1), (2, 2), (3, 1), (3, 2), (3, 3), (4, 1), (4, 2), (4, 3)]
        fc_l = [1,3]
        weights = {}
